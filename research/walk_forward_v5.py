"""
walk_forward_v5.py — More Trades + Tight Risk
============================================================
Key insight from iterations:
  v1 (baseline): -1.67%, MaxDD -14%  (too many bad pairs)
  v2 (all filters): +11.59%, MaxDD -2% (great DD control, too few entries)
  v3 (velocity+DD stop): +3.51%, MaxDD -10% (over-filtered)
  v4 (just pair filter): -11.55% (pair filter alone not enough)

Best combo so far was v2. Problem: too few trades (74).
Strategy: Keep v2's dual-test + stability + DD stop.
BUT lower entry threshold to z=1.5 to get MORE entries.
Also try 5-pair maximum (best pairs only, less dilution).
============================================================
"""
...
REBALANCE_DAYS = 126       # 6 months (v2 setting that worked)
WARMUP_DAYS = 150
Z_ENTRY = 1.5              # Lower → more entries (was 2.0)
Z_EXIT = 0.5
LOOKBACK = 60
Z_STOP = 3.5               # Tighter stop (was 4.0)
VELOCITY_LOOKBACK = 3      # Light velocity filter
PAIR_DD_STOP = 0.08
MIN_STAB = 60.0
MAX_PAIRS = 5              # Only best 5 (less dilution from weak pairs)


def generate_signals_with_velocity(zscore, entry_thresh, exit_thresh, vel_lb):
    """Entry with optional velocity confirmation."""
    position = 0
        ...
        if position == 0:
            if z < -entry_thresh:
                if vel_lb > 0 and i >= vel_lb:
                    if zscore.iloc[i] > zscore.iloc[i - vel_lb]:
                        position = 1
                else:
                    position = 1
            elif z > entry_thresh:
...
if __name__ == "__main__":
    print("=" * 60)
    print("WALK-FORWARD v5 — More Trades + Tight Risk")
    print("=" * 60)
    print(f"  z_entry={Z_ENTRY} (lower=more trades), z_exit={Z_EXIT}")
    print(f"  velocity={VELOCITY_LOOKBACK}d, pair_DD_stop={PAIR_DD_STOP*100:.0f}%")
    print(f"  max_pairs={MAX_PAIRS} (concentrated), min_stab={MIN_STAB}%")
    print(f"  Rebalance: {REBALANCE_DAYS}d\n")

    prices = load_data(force_refresh=False)
    dates = prices.index
    print(f"  Data: {dates[0].date()} → {dates[-1].date()}")
    ...
    for i, ri in enumerate(rb_indices):
        ...
        print(f"{len(sel)}p | Ret={m['total_return']:+.1f}% | Sh={m['sharpe_ratio']:.2f} | "
              f"DD={m['max_drawdown']:.1f}% | WR={m.get('win_rate',0):.0f}%")
    ...
    sdf.to_csv("data/walk_forward_v5_results.csv", index=False)
    print(f"\n💾 Saved")
